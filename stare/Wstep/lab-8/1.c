...
main () {
  char znak;
  int liczniki[DL_ALFABETU], i, j, x, a[DL_ALFABETU];
  ...
  for(i=DL_ALFABETU; i>1; i++){
   for(j=1; j<i; j++){
    while(licznik[DL_ALFABETU]<licznik[DL_ALFABETU-1]){
    x=licznik[DL_ALFABETU-1]; 
    licznik[DL_ALFABETU-1]=licznik[DL_ALFABETU]; 
    licznik[DL_ALFABETU]=x;
    }
   } 
  }
  printf("\nSTATYSTYKA:\n");
